...
/*
  if($link === 'userdata'){
    ...
    $conta_elementi = 'SELECT COUNT(*) AS num_dati
                       FROM dati JOIN dispositivi JOIN utenti
                       ON dati.id_d = dispositivi.id_disp AND utenti.id = dispositivi.id_u
                       WHERE utenti.nick = "' . $_SESSION['session_user'] . '"'; // da modificare
    ...
    if($res->rowCount() > 0){
        ...
        if($num_pagine > $pagina){
            echo '<a class="piu" href="?link=userdata&pagina='. ($pagina+1) . '#tabelladati">Pagina successiva '.($pagina+1).'>> </a>';
        }
    }
    echo '</center><br></div>';
  } else if ($link === 'alluserdata'){
